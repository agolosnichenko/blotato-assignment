 ...
 *
 * `graph-client.ts` surfaces exactly two shapes of failure: a {@link GraphHttpError} when the
 * Graph API answered with a non-2xx status (the request definitely reached Meta), or whatever
 * `fetch` itself throws when no response was ever received. `fetch` in Node 22 is built on
 * undici, and a failed `fetch` is a `TypeError` whose `.cause` carries undici's own error object
 * — that `.cause` is what tells "never sent" apart from "sent, then the connection died", rather
 * than any guess from a message string.
 */
...
function classifyHttpFailure(error: GraphHttpError): Error {
 ...
 */
export function classifyGraphFailure(error: unknown): Error {
