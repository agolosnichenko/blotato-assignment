...
|-------|---------|---------|
| `comment_sync_targets` | The per-post refresh schedule (**Refresh target**). A row is created from the post-published event, and also on the first ingested comment for a post not published through the platform — which is the only way an external post becomes tracked (§7.3, FR-018) | `id`, `workspace_id`, `social_account_id`, `post_id` (null for external posts), `platform_post_id`, `last_synced_at`, `next_sync_at` (null = deactivated), `last_error`, `manual_cooldown_until`, `age_anchor_at` (not null — the instant the §7.3 age bands are measured from; see root `spec.md` §18). `UNIQUE (social_account_id, platform_post_id)` |
